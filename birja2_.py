...
class Trading_3():
    def __init__(self,render_mode=None,other_param=2,balance=100):
        ...
        logging.basicConfig(filename=r"/content/drive/MyDrive/Colab Notebooks/log_0.txt",
            level=logging.INFO,
            format='%(asctime)s – %(name)s – %(levelname)s – %(message)s',
            datefmt='%d-%b-%y %H:%M:%S %p',
            filemode='w',encoding='utf-8')  # по умолчанию)
    ...
    def trader(self):
        ...
        if (low*self.risk_k<=self.stop*self.direction or high*self.risk_k<=self.stop*self.direction) and self.open_k:
            ...
            if pr==0: 
                ...
                logging.info(f'2)balanse:{-1*self.risk_per_trade*self.risk_k- self.risk_per_trade*self.risk_k*0.00045 }|take:{self.take}|stop:{self.stop}|dir:{self.direction}')
                print(f'2)balanse:{-1*self.risk_per_trade*self.risk_k- self.risk_per_trade*self.risk_k*0.00045 }|take:{self.take}|stop:{self.stop}|dir:{self.direction}')
            else:
                reward-=2
                logging.info('2)AAAAAARRR!!!',f'|pr:{pr}')
                print('2)AAAAAARRR!!!',f'|pr:{pr}')
        ...
        if (high*self.risk_k>=self.take*self.direction or low*self.risk_k>=self.take*self.direction) and self.open_k:
            if pr==0: 
                ...
                print(f'2)balanse:{abs(self.take/self.stop)*self.risk_per_trade- self.risk_per_trade*self.risk_k*0.00045}|take:{self.take}|stop:{self.stop}|dir:{self.direction}')
                logging.info(f'2)balanse:{abs(self.take/self.stop)*self.risk_per_trade- self.risk_per_trade*self.risk_k*0.00045}|take:{self.take}|stop:{self.stop}|dir:{self.direction}')
            else:
                reward-=2
                print('2)AAAAAARRR!!!',f'|pr:{pr}')
                logging.info('2)AAAAAARRR!!!',f'|pr:{pr}')
        ...
        if action==0 and self.open_k and self.prise!=0:
            ...
            if self.direction==diract or diract==0:
                ...
                if self.prise!=0:
                    ...
                    print(f'2)delta:{delta}|balanse:{(-1 if delta*self.risk_per_trade*self.risk_k <=-1 else delta*self.risk_per_trade*self.risk_k) -0.0007*self.risk_per_trade*self.risk_k}|take:{self.take}|stop:{self.stop}|dir:{self.direction}')
                    logging.info(f'2)delta:{delta}|balanse:{(-1 if delta*self.risk_per_trade*self.risk_k <=-1 else delta*self.risk_per_trade*self.risk_k) -0.0007*self.risk_per_trade*self.risk_k}|take:{self.take}|stop:{self.stop}|dir:{self.direction}')
    ...
    def end_episode(self,reward)->any:
            ...
            print(f'2)trade: {self.trade_count}| trade_: {self.trades}| balanse: {self.balance} | reward:{reward}| edn:{self.balance-self.balance_start}')
            logging.info(f'2)trade: {self.trade_count}| trade_: {self.trades}| balanse: {self.balance} | reward:{reward}| edn:{self.balance-self.balance_start}')
            self.open=0
            self.take=0
            self.stop=0
            self.prise=0
    ...
    def step(self,action=0,take=0,stop=0,open=0): 
        ...
        if self.index1>len(self.data_15)-1 or self.start1>self.data_15['timestamp'].iloc[-1]:
            ...
            logging.info(f"{self.index1}:{self.start1}|last:{len(self.data_15)-1}:{self.data_15['timestamp'].iloc[-1]}")
            while True:
                t=datetime.datetime.now(datetime.timezone.utc)
                dat,iz=self.pars(datetime.datetime(year=t.year,month=t.month,day=t.day,hour=t.hour,minute=15*(t.minute//15),tzinfo=datetime.timezone.utc)-datetime.timedelta(minutes=1))
                if iz:
                    print(f'2)index1:{self.index1}|data15:{dat['15m']['timestamp'][-3:]}')
                    logging.info(f'2)index1:{self.index1}|data15:{dat['15m']['timestamp'][-3:]}')
                    self.data_15=dat['15m']
                    self.data_1=dat['1w']
                    self.data_4=dat['4h']
                    break
                    ...
                    if ui:
                        ...
                        print(f'2)sleep:{datetime.timedelta(seconds=max(time_sleep-1,0.1))}| now:{t}| max:{max_t}|t:{t}')
                        logging.info(f'2)sleep:{datetime.timedelta(seconds=max(time_sleep-1,0.1))}| now:{t}| max:{max_t}|t:{t}')
                        sleep(max(time_sleep-1,0.1))
                        ui=True
        #print(f'2)ind:{self.index1}| time:{self.start1}')
        ...
        if truncated :
            ...
            print(f'2)reward end:{reward}')
            logging.info(f'2)reward end:{reward}')
