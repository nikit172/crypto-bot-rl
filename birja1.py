...
class Trading_3():
    def __init__(self,render_mode=None,other_param=2,balance=100):
        ...
        path1=Path("from_3_3.parquet")  # 15m
        path2=Path("from_2_3.parquet") # 4h
        path3=Path("from_1_3.parquet") # 1w
        #path1=Path("/content/drive/MyDrive/Colab Notebooks/from_3_3.parquet")  # 15m
        #path2=Path("/content/drive/MyDrive/Colab Notebooks/from_2_3.parquet") # 4h
        #path3=Path("/content/drive/MyDrive/Colab Notebooks/from_1_3.parquet") # 1w
        ...
        self.start2=datetime.datetime.fromisoformat('2018-12-13 00:00:00+00:00')
        #self.start2=datetime.datetime.fromisoformat('')
        ...
        self.current_step=self.index1

        self.render_mode=render_mode
        self.len_data=len(self.data_15)-690
    ...
    def trader(self):
        ...
        if (high*self.risk_k>=self.take*self.direction or low*self.risk_k>=self.take*self.direction) and self.open_k:
            if pr==0: 
                reward+=1 +17*abs(self.take/self.stop)*self.risk_per_trade- self.risk_per_trade*self.risk_k*0.00045#////////
    ...
    def _trade(self,action,stop,take,open):
        ...
        if action==0 and self.open_k and self.prise!=0:
            ...
            if self.direction==diract or diract==0:
                ...
                reward+=(-1 if delta*self.risk_per_trade*self.risk_k <=-1 else delta*self.risk_per_trade*self.risk_k)*(17 if delta>0 else 0 if delta<0 else 0) -0.0007*self.risk_per_trade*self.risk_k
                reward-=0.
            ...
            if self.prise!=0 and self.open_k:
                ...
                #print('close_per_stop:',close_per_stop,'| delta:',delta,'|delta balanse:',delta*self.risk_per_trade*self.risk_k -0.0007*self.risk_per_trade*self.risk_k)
                reward+=(-1 if delta*self.risk_per_trade*self.risk_k <=-1 else delta*self.risk_per_trade*self.risk_k)*(17 if delta>0 else 0 if delta<0 else 0) -0.0007*self.risk_per_trade*self.risk_k-0.1
    ...
    def step(self,action=0,take=0,stop=0,open=0): 
        ...
        done= self.index1>=(self.len_data-10)
